...
const bannerInfos: BannerInfo[] = [
  {
    id: "ai-automation",
    desktopText:
      "Host a world-class event, effortlessly.",
    mobileText: "Host world-class events, effortlessly.",
    linkText: "Launch now",
    linkUrl: "#",
  },
  {
    id: "hackathon-explore",
    desktopText:
      "Explore upcoming hackathons, join a team, and compete for top prizes.",
    mobileText: "Explore hackathons and prove your skill.",
    linkText: "Join Now",
    linkUrl: "#",
  },
  {
    id: "verse-warden",
    desktopText: "Introducing The Verse Warden. It guarantees a level playing field for every participant.",
    mobileText: "The Verse Warden: Fair competition for all.",
    linkText: "Install",
    linkUrl: "",
  },
]
...
export default function AnnouncementBanner({ onVisibilityChange }: AnnouncementBannerProps) {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 bg-black border-b border-white/10">
      ...
      <div
        className="flex items-center justify-center px-4 py-2 sm:py-3 max-w-7xl mx-auto min-h-11 sm:min-h-[52px]"
      ...
      >
        ...
        >
          ...
          <div
            className={`w-1 h-4 bg-linear-to-b from-cyan-400 via-orange-400 to-pink-400 rounded-full transition-all duration-300 ease-out motion-reduce:transition-none ${
        ...
        <button
          onClick={handleClose}
          className="absolute right-4 p-1.5 text-white/50 hover:text-white/80 hover:bg-white/5 rounded-md transition-all duration-200 shrink-0 motion-reduce:transition-none"
