...
export default function InstallModal({ isOpen, onClose }: InstallModalProps) {
  ...
  // Early return if no active option (should never happen but TypeScript safety)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start md:items-center justify-center p-4 bg-black/80 backdrop-blur-sm overflow-y-auto">
      <div
        ref={modalRef}
        className="relative w-full max-w-2xl bg-black border border-white/10 rounded-2xl shadow-2xl my-4 md:my-0"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Sticky header with close button */}
        <div className="sticky top-0 z-10 flex items-center justify-between p-4 md:p-6 bg-black rounded-t-2xl border-b border-white/10">
          <h2
