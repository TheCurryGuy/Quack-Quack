...
export default function Navbar({ isBannerVisible = true }: NavbarProps) {
  ...
  return (
    <div
      className={`fixed ${isBannerVisible ? "top-11 sm:top-12" : "top-0"} left-0 right-0 z-50 flex justify-center px-6 md:px-8 transition-all duration-300`}
    >
      <div className="w-[calc(100%-24px)] max-w-[1400px] mt-2">
        <nav
          className="flex items-center justify-between p-2 h-16 bg-black rounded-2xl text-white font-geist"
          style={navStyle}
        >
        ...
        {/* Mobile Menu */}
        <div
          className={`fixed top-[76px] right-6 w-[calc(100%-48px)] max-w-[400px] bg-black border border-[#1a1a1a] rounded-2xl shadow-lg z-50 transform transition-transform duration-300 ease-in-out ${
