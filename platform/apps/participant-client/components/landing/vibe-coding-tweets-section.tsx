...
const tweets: Tweet[] = [
  ...
  {
    text: "The team roster got corrupted — this platform saved the day!",
    handle: "@bnj",
    name: "Ben South",
  ...
  {
    text: "Finally, a tool that automatically manages AI-assisted team formation.",
    handle: "@tom_doerr",
    name: "Tom Dörr",
  ...
  {
    text: "No more chaos — I can roll back changes instantly when AI suggestions fail.",
    handle: "@thekitze",
    name: "Kitze",
  ...
  {
    text: "Cursor, GitHub, Slack integrations — everything I need to run events smoothly.",
    handle: "@iannuttall",
    name: "Ian Nuttall",
...
export default function VibeCodingTweetsSection({ onOpenInstall }: VibeCodingTweetsSectionProps) {
  ...
  const featuredTweet = tweets[0]
  if (!featuredTweet) return null
              ...
              <div className="space-y-6 sm:space-y-8">
                ...
                <div className="max-w-4xl mx-auto">
                  <a
                    ...
                    className="group block relative transition-all duration-300 cursor-pointer rounded-2xl sm:rounded-[20px] p-3 sm:p-6 md:p-10 bg-white/15 border border-white/30 sm:border-2 hover:border-white/50 hover:bg-white/20 backdrop-blur-sm"
                  >
                    {/* Original Tweet (if retweet) */}
                    {featuredTweet.isRetweet && featuredTweet.originalTweet && (
                      <div className="mb-4 p-4 bg-white/5 border border-white/10 rounded-lg">
                        ...
                        >
                          {featuredTweet.originalTweet.text}
                        </p>
                        <p
                        ...
                        >
                          {featuredTweet.originalTweet.handle}
                        </p>
                      </div>
                    )}
                    ...
                    <div className="mb-3 sm:mb-4 md:mb-8">
                      ...
                      >
                        {expandedTweets.has(0)
                          ? featuredTweet.text.split("\n").map((line, i) => (
                              <span key={i}>
                                {line}
                                {i < featuredTweet.text.split("\n").length - 1 && <br />}
                              </span>
                            ))
                          : truncateText(featuredTweet.text, 150)}
                        {featuredTweet.text.length > 150 && (
                          <button
                    ...
                    <div className="flex items-center justify-between flex-col sm:flex-row gap-2 sm:gap-0">
                      <div className="flex items-center gap-2 sm:gap-3 md:gap-4 w-full sm:w-auto">
                        <div className="w-8 h-8 sm:w-10 sm:h-10 md:w-14 md:h-14 rounded-full overflow-hidden border border-white/30 sm:border-2 bg-white/10">
                          <img
                            src={featuredTweet.profileImage || "/placeholder.svg"}
                            alt={`${featuredTweet.name} profile`}
                  ...
                  {tweets.slice(1).map((tweet, index) => {
                    ...
                    return (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="group block relative transition-all duration-300 cursor-pointer rounded-2xl p-4 sm:p-6 bg-white/10 border border-white/20 hover:bg-white/15 hover:border-white/30 break-inside-avoid mb-4 sm:mb-6 backdrop-blur-sm"
                        style={{
