...
export default function AgenticAISearchSection({ onOpenInstall }: AgenticAISearchSectionProps) {
  const [activeQuery, setActiveQuery] = useState<SearchQuery | undefined>(searchQueries[0])
  ...
  useEffect(() => {
    const preloadVideos = async () => {
      ...
      for (const query of searchQueries) {
        if (searchQueries[0] && query.id === searchQueries[0].id) continue
        ...
        try {
          ...
          videoCacheRef.current[query.id] = video
          ...
          video.onerror = () => {
            if (query.fallbackSrc && videoCacheRef.current[query.id]) {
              videoCacheRef.current[query.id]!.src = query.fallbackSrc
            }
          }
        } catch (error) {
    ...
    return () => {
      Object.keys(videoCacheRef.current).forEach((key) => {
        const video = videoCacheRef.current[key]
        if (video) {
          video.pause()
          video.src = ""
          // Don't call video.load() as it can cause issues
          video.onerror = null
        }
      })
  ...
  return (
    <div className="my-24">
      ...
      <div className="text-center mb-6 md:mb-12 px-4">
        ...
        >
          Transform complex event logistics into a seamless, automated workflow from a single dashboard.
        </p>
      </div>
