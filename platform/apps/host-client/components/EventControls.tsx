...
import { Play, Download, FileText, CheckCircle2, AlertCircle } from "lucide-react"
        ...
        {hackathon.status === "LIVE" && (
          <div className="space-y-4">
            <div className="flex items-start gap-3 p-4 bg-green-50 border border-green-200 rounded-lg">
              ...
              <div>
                <p className="font-semibold text-green-900">Hackathon is Live!</p>
                {isHackathonOver ? (
                  <p className="text-sm text-green-700 mt-1">
                    The scheduled duration is over. You can now export submissions and announce winners.
                  </p>
                ) : (
                  <p className="text-sm text-green-700 mt-1">Ends on: {endTime?.toLocaleString()}</p>
                )}
              </div>
            </div>
          </div>
